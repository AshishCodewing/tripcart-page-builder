        ...
        <TooltipProvider delay={300}>
          <ButtonGroup className="rounded-lg bg-white">
            ...
            {/* The content slot is removable:false (so `canDelete` is off),
                so this is its only delete affordance. The acknowledgement
                prompt is supplied globally by ContentSlotDeleteGuard, which
                intercepts this `.remove()` via `component:remove:before`. */}
            {isContentSlot && (
              <Tooltip>
                <TooltipTrigger
                  render={
