...
| `render-root.tsx` | `renderRoot` — mounts a React root on the iframe `document.body`; cleans up on frame unload / window `pagehide` (unmount queued as a microtask — React 19 forbids sync unmount mid-commit). |
