...
- Precise per-subtree style extraction in convert-to-template (replace the over-inclusive snapshot). See §6.
- Inline the resolved template tree as locked children in the editor canvas (so a converted header shows the real content on-canvas, not just the slug placeholder). See §7.
- **Register tenant templates as Block Manager entries** so users can drag templates from the sidebar like any other block — closes the "templates are invisible from the canvas" gap. See §8.
- **Slim `Template.data` to `{ component, styles }`** at the IO boundary. Removes the awkward `{ pages: [{ frames: [{ component }] }], styles: [] }` reshape in two code paths; the editor still sees the full project shape via a wrap/unwrap adapter. See §9.
...
Order below is "what unblocks what." Reorder as priorities shift. **Sequence note**: §9 (slim shape) should land before §8 (block registration) — the block snippet is cleaner with `tpl.data.component` than with the full project walk.
