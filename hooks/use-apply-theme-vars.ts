 ...
 * (Popover, Tooltip, etc.) portal into <body>, so a scoped wrapper
 * wouldn't cover them. On unmount we remove every key we wrote so
 * other routes (preview, blog) aren't polluted by editor-only tokens.
 */
...
export function useApplyThemeVars(): void {
  useEffect(() => {
    ...
    const apply = (): void => {
      const { rootVars } = compileTheme(themeStore.getTheme())
