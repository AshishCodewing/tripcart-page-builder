...
//
// Anyone (panel UI, plugin, future plugin API) can:
...
//
// The design-system plugin subscribes to this store and re-injects
// the canvas :root CSS variables on every change. The Theme panel
// subscribes via useTheme() to render current values.
...
const cloneTheme = (theme: TokenSchema): TokenSchema =>
