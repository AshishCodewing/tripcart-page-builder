...
import { crawlDocs, type RawPage } from "@/lib/rag/scrape"
...
// Guide + Reference live under /docs/ (single long pages); examples under
// /examples/. Two separate crawl roots since they don't share a path prefix.
// The changelog is excluded per scope.
const TARGETS = [
  {
    rootUrl: "https://prosemirror.net/docs/",
    pathPrefix: "/docs/",
    excludePathPrefixes: ["/docs/changelog"],
  },
  {
    rootUrl: "https://prosemirror.net/examples/",
    pathPrefix: "/examples/",
    excludePathPrefixes: [] as string[],
  },
]
...
async function main(): Promise<void> {
  ...
  console.log(
    `\n[ingest:prosemirror] starting (${args.dryRun ? "DRY RUN" : "live"}, max ${args.maxPages} pages/target)\n`
  )

  const rawPages: RawPage[] = []
  const seenUrls = new Set<string>()
  for (const target of TARGETS) {
    console.log(`[ingest:prosemirror] crawling ${target.rootUrl}…`)
    const pages = await crawlDocs({
      rootUrl: target.rootUrl,
      pathPrefix: target.pathPrefix,
      excludePathPrefixes: target.excludePathPrefixes,
      maxPages: args.maxPages,
      onPage: (url, total) => {
        if (total % 10 === 0) console.log(`  fetched ${total}: ${url}`)
      },
    })
    // Dedupe across targets (a URL could be reachable from both crawls).
  ...
  console.log(`[ingest:prosemirror] crawled ${rawPages.length} pages total\n`)

  if (rawPages.length === 0) {
    console.warn("[ingest:prosemirror] no pages — aborting")
    return
  }

  console.log("[ingest:prosemirror] cleaning + splitting…")
  ...
  console.log(
    `[ingest:prosemirror] produced ${chunks.length} chunks from ${cleanedPages.length} pages`
  )
  ...
  if (chunks.length === 0) {
    console.warn("[ingest:prosemirror] no chunks produced — aborting")
    return
  }

  if (args.dryRun) {
    console.log("[ingest:prosemirror] DRY RUN — sample chunks:\n")
    ...
    console.log(
      "[ingest:prosemirror] dry run complete — no DB writes, no embedding calls."
    )
    return
  }

  console.log("[ingest:prosemirror] checking which chunks need embedding…")
  ...
  if (newChunks.length > 0) {
    console.log("[ingest:prosemirror] embedding…")
    const embeddings = await embedBatch(newChunks.map((c) => c.content))
    console.log("[ingest:prosemirror] inserting chunks…")
    const inserted = await insertChunks(newChunks, embeddings, SOURCE)
    console.log(`         inserted ${inserted}\n`)
  }

  console.log("[ingest:prosemirror] upserting chunk→url rows…")
  const touched = await upsertChunkUrls(chunks)
  console.log(`         touched ${touched}\n`)

  console.log("[ingest:prosemirror] done.")
}
